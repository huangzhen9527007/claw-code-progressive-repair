...
>
> 基于Claude Code CLI逆向还原项目的二次开发版本，新增OpenAI兼容API适配层和微信远程控制桥接。本版本在adoresever/cloud-code项目基础上进行了五大核心改进，并修复了Hookify插件的Windows兼容性问题，集成了完整的Memory Tools记忆系统管理工具。
...
| **Bun构建兼容性** | **单一启动方式** | **多包管理器启动支持** | **npm/pnpm/yarn全面支持** |
...
9. **构建兼容性增强**：Bun-target-build兼容性问题解决 + 多包管理器启动支持
...
- **Memory Tools 插件** — 完整的记忆系统管理工具，支持项目特性识别、分类问题检测和修复
- 支持文字、图片、文件、语音、视频的收发
...
### Memory Tools - 记忆系统管理工具

本项目集成了 **Memory Tools**，这是一个全面的记忆系统管理插件，帮助维护和修复记忆系统完整性。

#### 功能特点
- **项目特性识别**：智能区分项目特性与真正问题，避免误判
- **7个分类问题检测**：完整检测中文文件名、缺少frontmatter、类型不匹配等问题
- **智能技术术语识别**：识别合理的混合语言描述（如技术术语与中文混合）
- **灵活命名约定**：支持多种合理的命名模式（kebab-case、snake_case、PascalCase等）
- **误报最小化**：从72个误判问题减少到1个真正问题，提高工具可信度

#### 核心改进：项目特性识别
- **问题识别**：正确识别"英文文件名但描述包含中文"是项目特性而非问题
- **技术术语库**：建立60+个技术术语和20+个模式匹配库
- **智能检测**：使用上下文感知的检测逻辑，理解项目特定约定
- **经验记录**：将完整的项目特性识别经验记录到记忆系统中
...
```bash
# 运行交互式菜单
memory_tools_menu.bat  # Windows
# 或
python main.py        # 选择菜单选项
```
...
#### 技术特性
- **UTF-8编码支持**：完全支持中文路径和文件名
- **跨平台兼容**：Windows/Linux/macOS 全平台支持
- **向后兼容**：支持所有旧版本命令行参数
- **模块化设计**：代码结构清晰，易于维护和扩展

详细文档请参考 `memory-tools/` 目录中的相关文档。
...
**注意**：项目使用 `--target bun` 构建，构建后的 `dist/cli.js` 文件包含 Bun 特定的运行时特性，无法直接用 Node.js 运行。请使用上述包管理器脚本启动。
...
**版本信息**：增强版 v1.0.6  
**更新日期**：2026-04-21  
**主要改进**：五大核心改进 + Hookify插件Windows兼容性修复 + WeChat CLI集成与问题诊断 + MemPalace插件版本同步与配置优化 + Claude-mem记忆系统路径问题解决方案 + Memory Tools插件完全整合与增强 + Memory Tools项目特性识别能力提升 + Bun构建兼容性与多包管理器启动支持，全面优化国产模型兼容性、插件兼容性、记忆系统稳定性、工具生态系统、项目特性识别能力和构建兼容性
...
记忆系统诊断工具是一个专门用于检查和维护Claude Code记忆系统的Python工具，提供全面的记忆系统健康检查和修复功能。
...
### 2026-04-21 v1.0.7 - Memory Tools项目特性识别增强
- **项目特性识别能力**：完善memory-tools工具的项目特性识别能力，区分项目特性与真正问题
- **混合语言描述智能检测**：改进混合语言描述检测逻辑，识别合理的混合语言（如技术术语）
- **命名约定灵活性增强**：支持多种合理的命名模式（kebab-case、snake_case、PascalCase等）
- **技术术语库建立**：添加60+个技术术语和20+个模式匹配，提高检测准确性
- **误报最小化**：从72个误判问题减少到1个真正问题，提高工具可信度
- **完整经验记录**：将项目特性识别经验详细记录到记忆系统中

### 2026-04-21 v1.0.6 - Bun构建兼容性与多包管理器启动支持
- **Bun-target-build兼容性问题解决**：分析并解决使用 `--target bun` 构建的兼容性问题
- **多包管理器启动支持**：添加 npm、pnpm、yarn 等多种包管理器启动脚本
- **统一启动接口**：提供 `cli`、`start`、`run`、`cloud-code`、`code`、`cc` 等多个别名脚本
- **技术限制说明**：明确 Bun 特定构建文件无法在 Node.js 中运行的限制
- **最佳实践指导**：在 CLAUDE.md 中说明正确的启动方式和使用方法
