...
class ClaudeAutoMemorySaver:
    ...
    def extract_complete_conversation(self, session_data):
        """提取完整对话内容"""
        ...
        for entry in session_data:
            ...
            if entry_type == "user" and "message" in entry:
                msg = entry["message"]
                conversation_entry["content"] = msg
                ...
                conversation_entry["content"] = entry.get("content", {})

            complete_conversation.append(conversation_entry)
    ...
    def create_complete_memory_content(self, project_name, file_name, complete_conversation, file_path=None):
        """创建完整的记忆内容 - 保存所有原始信息"""
        ...
        # 统计信息
        total_entries = len(complete_conversation)
        user_entries = sum(1 for entry in complete_conversation if entry["type"] == "user")
        assistant_entries = sum(1 for entry in complete_conversation if entry["type"] == "assistant")
        system_entries = sum(1 for entry in complete_conversation if entry["type"] == "system")

        # 创建完整的内容
        content = f"""# Claude Code 完整会话记忆
...
- **总条目数**: {total_entries}
- **用户消息**: {user_entries} 条
- **助手响应**: {assistant_entries} 条
- **系统消息**: {system_entries} 条
...
"""
        ...
        # 添加每个对话条目
        for i, entry in enumerate(complete_conversation):
            entry_type = entry["type"]
            entry_time = entry.get("timestamp", "未知时间")
            session_id = entry.get("session_id", "")

            content += f"\n### 条目 {i+1}: {entry_type.upper()} - {entry_time}\n"
            ...
            if entry_content:
                if isinstance(entry_content, dict):
                    # 格式化JSON内容
                    formatted_content = json.dumps(entry_content, ensure_ascii=False, indent=2)
                    content += f"```json\n{formatted_content}\n```\n"
                elif isinstance(entry_content, str):
        ...
        content += f"\n## 元数据\n"
        content += f"- **保存时间**: {timestamp}\n"
        content += f"- **保存方式**: 完整原始数据保存\n"
        content += f"- **数据完整性**: 100% 完整（{total_entries} 个条目）\n"
        content += f"- **文件哈希**: {hashlib.md5(str(file_path).encode()).hexdigest() if file_path else 'N/A'}\n"
...
"""
    ...
    def process_sessions(self, max_files=20, complete_mode=True):
        """处理会话文件"""
        print(f"[SEARCH] 开始扫描会话文件 (最多 {max_files} 个)...")
        if complete_mode:
            print(f"[MODE] 完整原始数据保存模式")
        ...
        for file_info in new_files:
            ...
            if complete_mode:
                # 完整模式：提取完整对话内容
                complete_conversation = self.extract_complete_conversation(session_data)
                print(f"  提取: {len(complete_conversation)} 个完整对话条目")

                # 创建完整的记忆内容
                content = self.create_complete_memory_content(
                    project_name, file_path.name, complete_conversation, file_path
                )
            else:
                # 摘要模式：提取摘要
...
def main():
    ...
    parser.add_argument("--complete", action="store_true", help="Complete raw data mode (save all original information)")
    parser.add_argument("--complete", action="store_true", help="Complete raw data mode (save all original information)")
    ...
    if args.continuous:
        print("[WARNING] 完整模式不支持持续运行（文件会很大），使用摘要模式")
        saver.run_continuous(interval=args.interval)
    else:
        saver.process_sessions(max_files=args.max_files, complete_mode=args.complete)
