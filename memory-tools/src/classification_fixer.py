...
class ClassificationFixer:
    ...
    def _detect_mixed_language_issue(self, filename: str, frontmatter: Optional[Dict]) -> Optional[str]:
        ...
        注意：在这个项目中，英文文件名但描述包含中文是项目的特性，不是问题。
        此方法仅检测真正的混合语言问题，如中文文件名但描述只包含英文。
        ...
        has_english = self._contains_english(description)

        # 情况1: 中文文件名
        if contains_chinese(filename):
            # 中文文件名应该使用中文描述
            # 如果中文文件名但描述只包含英文，这可能是个问题
            if has_english and not has_chinese:
                return f"{filename}: 中文文件名但描述只包含英文 - '{description[:50]}...'"
            return None

        # 情况2: 英文文件名
        else:
            # 英文文件名可以使用中文描述（这是项目的特性）
            # 我们只标记真正的混合语言问题
            if has_chinese and has_english:
                # 检查是否是合理的混合（如技术术语）
                if self._is_reasonable_mixed_language(description):
                    return None
                return f"{filename}: 描述包含不合理的混合语言 - '{description[:50]}...'"
            return None
    ...
    def _contains_english(self, text: str) -> bool:
        ...
        Returns:
            bool: 如果包含英文字符则返回True
        """
        # 检查是否包含英文字母
        import re
        return bool(re.search(r'[a-zA-Z]', text))

    def _is_reasonable_mixed_language(self, description: str) -> bool:
        """检查混合语言描述是否合理

        在这个项目中，英文技术术语与中文描述混合是合理的。
        例如："Claude Code插件配置、注册和使用完整经验总结"

        Args:
            description: 描述文本

        Returns:
            bool: 如果是合理的混合语言则返回True
        """
        # 检查是否包含常见的技术术语
        tech_terms = [
            'Claude', 'Code', 'API', 'JSON', 'YAML', 'Python', 'JavaScript',
            'TypeScript', 'React', 'Node', 'Bun', 'Git', 'GitHub', 'Windows',
            'macOS', 'Linux', 'Docker', 'Kubernetes', 'AWS', 'Azure', 'GCP',
            'OpenAI', 'Anthropic', 'DeepSeek', 'WeChat', '微信', 'iLink',
            'Bot', 'CLI', 'TUI', 'GUI', 'SDK', 'MCP', '插件', '技能',
            'memory', 'tools', '分类', '检测', '修复', '智能', '命令行',
            '参数', '验证', '文档', '系统', '整合', 'LLM', 'Wiki', 'pattern',
            '递归', '搜索', '路径', '分隔符', '规范化', '兼容性', '配置',
            '简化', '房间', 'package', 'manager', 'launch', 'solution',
            'npm', 'pnpm', 'yarn', 'dist', 'cli', 'PATH', '环境变量',
            '配置', '指南', '检查', '清单', '补充', '终端', '安全', '管理'
        ]
        ...
        # 检查是否是常见的项目模式
        patterns = [
            r'Claude\s*Code', r'API\s*适配', r'JSON\s*格式', r'YAML\s*配置',
            r'Python\s*脚本', r'Windows\s*环境', r'微信\s*桥接', r'iLink\s*Bot',
            r'OpenAI\s*兼容', r'记忆\s*系统', r'插件\s*配置', r'技能\s*系统',
            r'memory\s*tools', r'分类\s*问题', r'检测\s*修复', r'命令行\s*参数',
            r'文档\s*系统', r'LLM\s*Wiki', r'递归\s*搜索', r'路径\s*分隔符',
            r'package\s*manager', r'PATH\s*环境变量', r'配置\s*指南'
        ]
    ...
    def _has_naming_inconsistency(self, filename: str) -> bool:
        ...
        注意：在这个项目中，某些大写字母的使用是合理的（如缩写、强调）。
        此方法只检测真正的问题，如同时使用下划线和连字符。
        ...
        # 3. 检查是否是合理的命名模式
        # 允许的模式：
        #   - kebab-case（连字符分隔）：memory-tools-enhancement
        #   - snake_case（下划线分隔）：memory_tools_enhancement
        #   - PascalCase（帕斯卡命名）：MemoryToolsEnhancement
        #   - camelCase（驼峰命名）：memoryToolsEnhancement
        #   - UPPER_CASE（全大写）：AUTO_MEMORY_SUMMARY
        #   - 混合大小写但合理的：Bun-target-build-compatibility

        # 检查是否是合理的混合大小写
        if name_without_ext != name_without_ext.lower():
            # 检查是否是合理的模式
            # 1. 全大写（可能是缩写）
            if name_without_ext.isupper():
                return False

            # 2. 帕斯卡命名（首字母大写）
            if re.match(r'^[A-Z][a-z]+([A-Z][a-z]+)*$', name_without_ext):
                return False

            # 3. 驼峰命名
            if re.match(r'^[a-z]+[A-Z][a-z]*([A-Z][a-z]*)*$', name_without_ext):
                return False

            # 4. 混合大小写但使用连字符（如Bun-target-build）
            if '-' in name_without_ext:
                # 检查每个部分是否合理
                ...
                for part in parts:
                    # 部分可以是：全小写、全大写、帕斯卡命名
                    if not (part.islower() or part.isupper() or
                           re.match(r'^[A-Z][a-z]+([A-Z][a-z]+)*$', part)):
                        all_parts_valid = False
                        break
                if all_parts_valid:
                    return False

            # 5. 混合大小写但使用下划线（如HOOKIFY_FIX_SUMMARY）
            if '_' in name_without_ext:
                ...
                for part in parts:
                    # 部分可以是：全大写、全小写、帕斯卡命名
                    if not (part.isupper() or part.islower() or
                           re.match(r'^[A-Z][a-z]+([A-Z][a-z]+)*$', part)):
                        all_parts_valid = False
                        break
                if all_parts_valid:
        ...
        # 如果不符合任何合理模式，则标记为问题
        return name_without_ext != name_without_ext.lower()
...
"""
    ...
    def _get_issue_type_name(self, issue_type: str) -> str:
        ...
        names = {
            ...
            "mixed_language_descriptions": "不合理的混合语言描述",
