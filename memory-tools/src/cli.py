...
def handle_check(args, memory_tools):
    ...
    if args.json:
        ...
        print("=" * 60)
        print("MEMORY SYSTEM CHECK - 记忆系统检查")
        print("=" * 60)
        print(f"Memory files: {file_count}")
        print(f"Index entries: {index_count}")

        if match_result:
            print("[OK] Counts match - 文件数量匹配")
        else:
            difference = abs(file_count - index_count)
            print(f"[ERROR] Counts don't match (difference: {difference}) - 文件数量不匹配 (差异: {difference})")
        ...
        if unindexed:
            print(f"\n[WARNING] Unindexed files: {len(unindexed)} - 未索引文件: {len(unindexed)}")
            for i, filename in enumerate(unindexed[:5], 1):
                print(f"   {i}. {filename}")
            if len(unindexed) > 5:
                print(f"   ... and {len(unindexed) - 5} more - 还有 {len(unindexed) - 5} 个")

        if extra:
            print(f"\n[WARNING] Extra indexes: {len(extra)} - 多余索引: {len(extra)}")
            for i, filename in enumerate(extra[:5], 1):
                print(f"   {i}. {filename}")
            if len(extra) > 5:
                print(f"   ... and {len(extra) - 5} more - 还有 {len(extra) - 5} 个")

        if duplicates:
            print(f"\n[WARNING] Duplicate indexes: {len(duplicates)} - 重复索引: {len(duplicates)}")
            for i, (filename, count) in enumerate(list(duplicates.items())[:5], 1):
                print(f"   {i}. {filename} (appears {count} times) - (出现 {count} 次)")
            if len(duplicates) > 5:
                print(f"   ... and {len(duplicates) - 5} more - 还有 {len(duplicates) - 5} 个")

        if not any([unindexed, extra, duplicates]):
            print("\n[OK] No obvious problems found - 未发现明显问题")
...
def handle_report(args, memory_tools):
    ...
    if args.json:
        ...
        print("=" * 60)
        print("MEMORY SYSTEM DIAGNOSTIC REPORT - 记忆系统诊断报告")
        print("=" * 60)
...
def handle_classify(args, memory_tools):
    ...
    if args.json:
        ...
        print("=" * 60)
        print("CLASSIFICATION ISSUES REPORT - 分类问题报告")
        print("=" * 60)
...
def handle_fix(args, memory_tools):
    ...
    print("=" * 60)
    if dry_run:
        print("FIX PLAN PREVIEW (DRY RUN) - 修复计划预览（试运行模式）")
        print("=" * 60)
        print("[INFO] This is a preview. No changes will be made.")
        print("[信息] 这是预览模式，不会进行实际修改。")
    else:
        print("EXECUTING FIXES - 执行修复")
        print("=" * 60)
        print("[WARNING] This will modify your memory system!")
        print("[警告] 这将修改您的记忆系统！")
        print("[INFO] Make sure you have a backup before proceeding.")
        print("[信息] 请确保在执行前已备份记忆系统。")

        # 添加确认提示
        try:
            response = input("\nDo you want to continue? (yes/no): ").strip().lower()
            if response not in ['yes', 'y']:
                print("Operation cancelled. - 操作已取消")
                return
...
def main():
    ...
    try:
        ...
        print(f"[ERROR] {e}")
        print("[ERROR] Memory system not found or not initialized.")
        print("[错误] 未找到记忆系统或记忆系统未初始化。")
        print("[INFO] Use --memory-dir parameter to specify the correct directory.")
        print("[信息] 使用 --memory-dir 参数指定正确的目录。")
        sys.exit(1)
