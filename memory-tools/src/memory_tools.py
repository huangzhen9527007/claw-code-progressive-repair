...
class MemoryTools:
    ...
    def print_report(self, report: Dict[str, Any] = None):
        ...
        print(f"  记忆文件数量: {stats['memory_files']}")
        print(f"  索引条目数量: {stats['index_entries']}")

        if stats["counts_match"]:
            print("  [正常] 数量匹配")
        else:
            diff = abs(stats['memory_files'] - stats['index_entries'])
            print(f"  [警告] 数量不匹配 (相差 {diff} 个)")
        ...
        if classification_problems:
            ...
            for issue_type, items in classification_issues.items():
                if issue_type != "naming_convention_analysis" and items:
                    ...
                    print(f"  {issue_name}: {len(items)}个")
                    total_issues += len(items)

            print(f"  总分类问题数: {total_issues}")
            print("  使用 --classification-report 查看详细分类问题")
